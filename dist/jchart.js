                ...
                _.each(_this.chartData.datasets, function(set, i) {
                    ...
                    _.each(set.data, function(d, j) {
                        var x = scale.x + cfg.barSetSpacing + scale.xHop * j + scale.barWidth * i + cfg.barSpacing * i + cfg.barBorderWidth * i,
                            ...
                            height = animPc * _this.calcOffset(d, scale.yScaleValue, scale.yHop) + (cfg.barBorderWidth / 2),
                            color = set.color,
                            borderColor, bgColor = _.hex2Rgb(color, 0.6);
        ...
        var Chart = function() {
            ...
            this.bindTouchEvents = function() {
                var touch = {},
                    ...
                    longTapTimeout, now, delta, offset,
    ...
    ;(function(_) {
        function Line(data, cfg) {
            ...
            this.drawLines = function(animPc) {
                ...
                var ctx = _this.ctx,
                    cfg = _this.config,
                    dataset = _this.chartData.datasets,
                    scale = _this.scaleData;
        ...
        function Pie(data, cfg) {
            ...
            function drawText(x, y, r, start, end, data) {
                ...
                var xaxis = Math.cos(middAngle) * dis + x,
                    yaxis = Math.sin(middAngle) * dis + y;
    ...
    ;(function(_) {
        function Polar(data, cfg) {
            ...
            this.drawScale = function() {
                ...
                //画角度分割线
                var len = this.data.labels.length,
                    rotateAngle = (2 * Math.PI) / len;
            ...
            this.drawAllSegments = function(animPc) {
                var startAngle = -Math.PI / 2,
                    angleStep = (Math.PI * 2) / this.data.datasets.length,
                    scaleAnim = 1,
                    rotateAnim = 1,
                    scale = this.scaleData,
                    cfg = this.config,
        ...
        function Radar(data, cfg) {
            ...
            this.drawScale = function() {
                var ctx = this.ctx,
                    cfg = this.config,
                    scale = this.scaleData,
                    scaleSize = cfg.scaleFont.size,
                    textSize = cfg.textFont.size,
                    dataLen = this.data.labels.length,
                    px = cfg.scaleBackdropPaddingX,
                    py = cfg.scaleBackdropPaddingY;
                //计算每条数据的角度
        ...
        function Scale() {
            ...
            this.calcDrawingSizes = function() {
                var maxSize = this.height,
                    widestX = 0,
                    scaleFontSize = this.config.scaleFont.size,
                    xLabelWidth = 0,
                    xLabelHeight = scaleFontSize,
                    labelRotate = 0,
                    dataLen = this.chartData.labels.length;
            ...
            this.drawScale = function() {
                ...
                _.each(this.chartData.labels, function(label, i) {
                    ctx.save();
                    var cx = scale.x + i * scale.xHop,
                        labelY = scale.y + P_X / 2,
